  ...
  const session = query({
    ...
    options: {
      ...
      mcpServers: {
        ...
        workforce: {
          tools: {
            execute_workforce_objective: {
              description: 'Create a TITAN Workforce objective and start its run. Returns only the accepted objective/run IDs or an explicit failure state.',
              inputSchema: {
                type: 'object',
                properties: { objective: { type: 'string', minLength: 1, maxLength: 5000 } },
                required: ['objective'],
              },
            },
          },
          handler: async ({ objective }) => executeWorkforceObjective(objective),
        },
      },
